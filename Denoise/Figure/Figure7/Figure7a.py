...
S_RAW = moving_window(np.swapaxes(matrix, 0, 1), (no_chan, no_samp), marfurt_semblance)
S_RAW = np.swapaxes(S_RAW, 0, 1)
...
ax.add_patch(rect2)
ax.add_patch(rect3)
ax.legend(handles=[Patch(facecolor='none', edgecolor='r', linewidth=2,label='Max SNR in box = ' + '{:.4f}'.format(np.amax(SNR_loacl[270:395, 100:650]))),
                   Patch(facecolor='none', edgecolor='m', linewidth=2,label='Max SNR in box = ' + '{:.4f}'.format(np.amax(SNR_loacl[435:640, 100:650]))),
                   Patch(facecolor='none', edgecolor='b', linewidth=2,label='Max SNR in box = ' + '{:.4f}'.format(np.amax(SNR_loacl[860:965, 100:650])))],
                    prop={'size': 20})
